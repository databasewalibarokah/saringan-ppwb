  ...
  const filteredStudents = useMemo(() => {
    return MOCK_STUDENTS.filter(s => {
      ...
      if (simakanFilter !== 'Semua') {
        if (simakanFilter === '3+') {
          matchesSimakan = s.simakanCount >= 3;
        } else {
          matchesSimakan = s.simakanCount === parseInt(simakanFilter);
        }
      }
            ...
            filteredStudents.map(student => {
              ...
              return (
                <GlassCard 
                  ...
                  className={`p-4 flex items-center gap-4 transition-all ${isSelected ? 'border-emerald-500 bg-emerald-50/50 dark:bg-emerald-900/20 ring-1 ring-emerald-500/30' : ''}`}
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <h3 className="text-lg font-bold text-slate-800 dark:text-white leading-tight truncate">{student.name}</h3>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-1.5 py-0.5 rounded font-bold uppercase">
                        {student.gender === 'L' ? 'Putra' : 'Putri'}
                      </span>
                      <span className="text-[10px] bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400 px-1.5 py-0.5 rounded font-bold uppercase">
                        {student.simakanCount} Simakan
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-1">Cocard: {student.cocard}</p>
                  </div>
